...
\newcommand{\stats}[6]{
  ...
  \bfseries{STR} & \bfseries{DEX} & \bfseries{CON} & \bfseries{INT} & \bfseries{WIS} & \bfseries{CHA} \\
  #1\mod{#1}&#2\mod{#2}&#3\mod{#3}&#4\mod{#4}&#5\mod{#5}&#6\mod{#6} \\
  \end{tabular}
}

\newcommand{\creatureName}[1]{{\huge \textrm{\textsc{#1}}}}
\newcommand{\ability}[2]{{\bfseries{#1}} #2}
...
%To hit and reach/range
\newcommand{\toH}[1]{{\bf{+#1}} to hit}
\newcommand{\reach}[1]{reach #1 ft.}
\newcommand{\rangeOne}[1]{range #1 ft.}
\newcommand{\rangeTwo}[2]{range #1/#2 ft.}

%calculate damage and hp based on dice and mods
\newcommand{\hit}[3]{\settemp{#3}
        \FPeval\result{trunc(#1*((#2+1)/2)+\tempval:0)}
        \hspace{-8pt}\textbf{\result\hspace{3pt}(#1d#2\pmz{#3}\hspace{-2pt})}} 
\newcommand{\hitpoints}[3]{\ability{Hit Points}{\hit{#1}{#2}{#3}}}
\newcommand{\damage}[4]{{\emph{Hit:}}\hit{#1}{#2}{#3}\da{#4} damage.}
